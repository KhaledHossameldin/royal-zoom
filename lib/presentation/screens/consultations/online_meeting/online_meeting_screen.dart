...
class OnlineMeetingScreen extends StatefulHookWidget {
  const OnlineMeetingScreen({super.key});

  @override
  State<OnlineMeetingScreen> createState() => _CallScreenState();
}

class _CallScreenState extends State<OnlineMeetingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    useEffect(() {
      Future<void>.microtask(() async {
        var token = generateJwt('Session Name', '1');
        try {
          ...
          AlertDialog(
            ...
            content: Text(appLocalizations.failedToJoinSession),
          );
          Future.delayed(const Duration(milliseconds: 1000))
            ...
            Navigator.popUntil(
              context,
              ModalRoute.withName(Routes.consultationDetails),
            );
    ...
    useEffect(() {
      ...
      final sessionNeedPasswordListener =
        ...
        showDialog<String>(
          ...
          builder: (BuildContext context) => AlertDialog(
            title: Text(appLocalizations.sessionNeedPassword),
      ...
      final sessionPasswordWrongListener =
        ...
        showDialog<String>(
          context: context,
          builder: (BuildContext context) => AlertDialog(
            title: Text(appLocalizations.sessionPasswordIncorrect),
        ...
        if (data['status'] == RecordingStatus.Start) {
          if (mySelf != null && !mySelf.isHost!) {
            showDialog<String>(
              context: context,
              builder: (BuildContext context) => AlertDialog(
                content: Text(appLocalizations.sessionIsRecording),
                actions: <Widget>[
                  TextButton(
                    onPressed: () async {
        ...
        switch (permissionType) {
          case SystemPermissionType.Camera:
            showDialog<String>(
              context: context,
              builder: (BuildContext context) => AlertDialog(
                title: Text(appLocalizations.cantAccessCamera),
                content: Text(appLocalizations.turnOnCameraPermission),
                actions: <Widget>[
                  TextButton(
          ...
          case SystemPermissionType.Microphone:
            showDialog<String>(
              context: context,
              builder: (BuildContext context) => AlertDialog(
                title: Text(appLocalizations.cantAccessMic),
                content: Text(
                  appLocalizations.turnOnCameraPermission,
                ),
                actions: <Widget>[
                  TextButton(
                    onPressed: () => Navigator.pop(context, 'OK'),
                    child: Text(appLocalizations.cantAccessCamera),
                  ),
                ],
              ),
            );
    ...
    Future<void> onPressAudio() async {
      ZoomVideoSdkUser? mySelf = await zoom.session.getMySelf();
      if (mySelf != null) {
    ...
    Future<void> onPressVideo() async {
      ZoomVideoSdkUser? mySelf = await zoom.session.getMySelf();
      if (mySelf != null) {
    ...
    Future<void> onPressShare() async {
    ...
    Future<void> onSelectedUserVolume(ZoomVideoSdkUser user) async {
    ...
    Future<void> onLeaveSession(bool isEndSession) async {
    ...
    Future<void> showLeaveOptions() async {
      ZoomVideoSdkUser? mySelf = await zoom.session.getMySelf();
    ...
    Future<void> onSelectedUser(ZoomVideoSdkUser user) async {
      setState(() {
